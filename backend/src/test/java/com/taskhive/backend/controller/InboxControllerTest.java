...
public class InboxControllerTest {
    ...
    public void InboxController_CreateProjectInvite_Returns_Project_Not_Owned_By_You() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/createProjectInvite").header("pid", 2).header("username", "TestUser")).andExpect(status().isInternalServerError()).andReturn();
    ...
    @Test
    @WithMockUser(username = "TestUser")
    public void InboxController_CreateProjectInvite_Returns_Cant_Create_Invite_For_Project_Invite() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/createProjectInvite").header("pid", 1).header("username", "TestUser")).andExpect(status().isInternalServerError()).andReturn();
    ...
    @Test
    @WithMockUser(username = "TestUser")
    public void ProjectController_CreateProjectInvite_Returns_TargetUser_Does_Not_Exist() throws Exception {
        //user with the username "NoSuchUser" has not been configured in the mock environment
        MvcResult result = mockMvc.perform(get("/createProjectInvite").header("pid", 1).header("username", "NoSuchUser")).andExpect(status().isInternalServerError()).andReturn();
    ...
    @Test
    @WithMockUser(username = "TestUser")
    public void ProjectController_CreateProjectInvite_Returns_TargetProject_Does_Not_Exist() throws Exception {

        //project with pid = 2 has not been configured in the mock environment
        MvcResult result = mockMvc.perform(get("/createProjectInvite").header("pid", 2).header("username", "")).andExpect(status().isInternalServerError()).andReturn();
    ...
    @Test
    @WithMockUser(username = "TestUser")
    public void ProjectController_CreateProjectInvite_Returns_Project_Could_Not_Be_Saved() throws Exception {
    ...
    @Test
    @WithMockUser(username = "TestUser")
    public void ProjectController_CreateProjectInvite_Returns_OK() throws Exception {
        AppUser anotherTestUser = AppUser.builder().uid(200).build();
